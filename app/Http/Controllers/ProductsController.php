...
class ProductsController extends Controller
{
    public function contactOwner(Request $request)
    {
        ...
        $lender = User::find($product->lender_id);
        ...
        $transaction->renter_id = $user->id;
        $transaction->product_id = $product->id;
        $transaction->from_date = $from_date;
        $transaction->to_date = $to_date;
        $transaction->status = 1;
        $transaction->save();

        return array('status' => 'success', 'name' => $lender->first_name . ' ' . $lender->last_name);
    }
